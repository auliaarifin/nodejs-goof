pipeline{
   ...
   stages {
       stage('Build'){
        ...
        stage('Build Docker Imgae and push to Docker Registry'){
           agent {
               docker {
                   image 'docker:dind'
        ...
        stage('Deploy Docker Image'){
            ...
            steps {
               withCredentials([sshUserPrivateKey(credentialsId: "DeploymentSSHKey", keyFileVariable: 'keyfile')]) {
                   sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no osboxes@10.10.10.10 "echo $DOCKERHUB_CREDENTIAL_PSW | docker login -u $DOCKERHUB_CREDENTIAL_USR --password-stdin"'
